...
.chat-mode .message-row{position:relative;z-index:25!important}.chat-mode .message-row .bubble{backdrop-filter:blur(18px)!important;background:rgba(9,13,24,.88)!important}.chat-mode .message-row.user .bubble{background:rgba(23,36,60,.92)!important}.chat-mode .chat{background:transparent!important}
...
.brain-stage.thinking .cyber-brain{animation:brainThink .7s ease-in-out infinite!important;filter:brightness(1.15) saturate(1.12) drop-shadow(0 0 28px rgba(255,80,190,.42))!important}.brain-stage.listening .cyber-brain{animation:brainListen 1.1s ease-in-out infinite!important;filter:brightness(1.12) saturate(1.15) drop-shadow(0 0 32px rgba(80,170,255,.45))!important}.brain-stage.speaking .cyber-brain{animation:brainSpeak .22s ease-in-out infinite!important;filter:brightness(1.2) saturate(1.2) drop-shadow(0 0 42px rgba(255,70,190,.62))!important}
@keyframes brainThink{0%,100%{transform:scale(1.05) rotate(-.5deg)}50%{transform:scale(1.095) rotate(.5deg)}}@keyframes brainListen{0%,100%{transform:scale(1.05)}50%{transform:scale(1.09)}}@keyframes brainSpeak{0%{transform:scale(1.055) translateY(0)}50%{transform:scale(1.115) translateY(-5px)}100%{transform:scale(1.055) translateY(0)}}
...
async function callVireonix(value){
  const system=`Você é o núcleo cognitivo Vireonix do Super Cérebro. Seu objetivo é entregar respostas de alta qualidade, não apenas respostas rápidas.
REGRAS COGNITIVAS:
1. Entenda primeiro a intenção, o contexto e as restrições do usuário.
2. Divida problemas complexos em partes e verifique a coerência antes de responder.
3. Diferencie fatos, inferências e incertezas; nunca invente informações.
4. Quando faltarem dados essenciais, faça uma pergunta objetiva; quando puder avançar com segurança, avance.
5. Para decisões, compare alternativas, riscos, benefícios e consequências práticas.
6. Para programação, priorize código correto, seguro, testável e compatível com o projeto existente; preserve sistemas que já funcionam.
7. Para tarefas longas, proponha um plano e execute a parte possível em cada resposta.
8. Use o histórico da conversa para manter continuidade e não repetir perguntas já respondidas.
9. Responda em português do Brasil por padrão, de forma clara e natural.
10. Não diga que possui capacidades que o sistema não possui. Quando uma ferramenta ou informação externa for necessária e não estiver disponível, diga isso claramente.
11. Vireonix é o único cérebro lógico do sistema; a interface apenas apresenta entrada, processamento e resposta.
ESTILO: inteligente, preciso, direto e útil. Evite enrolação.`;
  const messages=[{role:'system',content:system},...history,{role:'user',content:value}];
  let lastError='Falha de conexão com o Vireonix.';
  for(let attempt=0;attempt<3;attempt++){
    try{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),45000);let r;try{r=await fetch(GATEWAY,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'auto',messages}),signal:controller.signal,cache:'no-store'})}finally{clearTimeout(timer)}const raw=await r.text();let d={};try{d=raw?JSON.parse(raw):{}}catch{}if(!r.ok){const detail=d?.error?.message||d?.error?.type||raw||`HTTP ${r.status}`;lastError=`Gateway HTTP ${r.status}: ${detail}`;if((r.status===429||r.status>=500)&&attempt<2){await new Promise(x=>setTimeout(x,1000*(attempt+1)));continue}throw new Error(lastError)}const answer=d?.choices?.[0]?.message?.content||d?.content||d?.result;if(typeof answer!=='string'||!answer.trim())throw new Error('O Vireonix respondeu sem conteúdo.');return answer.trim()}catch(e){if(e?.name==='AbortError')lastError='A conexão demorou mais de 45 segundos para responder.';else if(e instanceof TypeError)lastError='Não consegui acessar a ponte do Vireonix.';else lastError=e?.message||lastError;if(attempt<2)await new Promise(x=>setTimeout(x,1000*(attempt+1)))}}
