...
// Captura em fase de captura para não ser sobrescrita pelo pequeno script visual do index.html.
document.addEventListener('change',e=>{if(e.target!==fileInput)return;const f=e.target.files?.[0];if(!f)return;if(!f.type.startsWith('image/')){alert('Escolha uma imagem ou print.');e.target.value='';return}selectedImage=f;const info=document.getElementById('attachmentInfo'),preview=document.getElementById('attachmentPreview');if(info)info.textContent=`🖼️ ${f.name} • ${Math.round(f.size/1024)} KB • pronto para leitura`;preview?.classList.add('show')},true);
